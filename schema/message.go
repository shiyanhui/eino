...
type Message struct {
	...
	// AccumulatedCompressedCreatedAt is the time when the summarization was performed.
	AccumulatedCompressedCreatedAt time.Time `json:"-"`
	...
	// ToolResultOffloadPath is the absolute file path where the tool result was offloaded to disk.
...
type TokenUsage struct {
	...
	// CompactCompletionTokens is the token count computed with compact mode enabled (offloaded tool results
	// are counted by file path length rather than content length). Cached separately from CompletionTokens
	// so that both compact and non-compact estimates can be reused without recomputing.
	CompactCompletionTokens int `json:"compact_completion_tokens,omitempty"`
}
